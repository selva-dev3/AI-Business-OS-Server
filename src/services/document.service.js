...
const list = async (companyId, query = {}) => {
  ...
  if (query.search) {
    const searchFilter = buildSearchQuery(query.search, ['name', 'description']);
...
const share = async (companyId, documentId, data, userId) => {
  const document = await Document.findOne({ _id: documentId, companyId });
  if (!document) throw new AppError(404, 'NOT_FOUND', 'Document not found');
...
const search = async (companyId, query = {}) => {
  ...
  if (query.q) {
    const searchFilter = buildSearchQuery(query.q, ['name', 'description', 'tags']);
    Object.assign(filter, searchFilter);
  }
  if (query.tags) {
