...
interface ForecastResult {
  ...
  forecastData: { period: string; forecast: number }[];
  model: {
    type: string;
    slope: number;
    intercept: number;
...
const chat = async (messages: ChatMessage[], context: string): Promise<ChatResult> => {
...
const getInsights = async (module: string, _data: Record<string, unknown>): Promise<Insight[]> => {
...
const summarize = async (entityType: string, entityId: string): Promise<SummaryResult> => {
...
const generateEmail = async (data: EmailData): Promise<EmailResult> => {
...
const extractDocument = async (file: FileInput): Promise<OCRResult> => {
...
const forecast = async (historicalData: HistoricalDataPoint[], periods: number): Promise<ForecastResult> => {
  if (historicalData.length < 2) {
    throw new AppError(400, 'BAD_REQUEST', 'At least 2 data points required for forecasting');
  }
  ...
  const futureData: { period: string; forecast: number }[] = [];
...
const parseResume = async (file: FileInput): Promise<ResumeParseResult> => {
  if (!file) throw new AppError(400, 'BAD_REQUEST', 'Resume file is required');

  const resumeData: ResumeParseResult = {
    fileName: file.originalname,
    processedAt: new Date().toISOString(),
    parsedData: {
      personalInfo: {
