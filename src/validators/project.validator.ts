...
export const addMemberSchema: Joi.ObjectSchema = Joi.object({
...
export const moveTaskSchema: Joi.ObjectSchema = Joi.object({
  status: Joi.string().valid('TODO', 'IN_PROGRESS', 'REVIEW', 'DONE').required(),
  position: Joi.number().integer().min(0).required(),
});

export const logTimeSchema: Joi.ObjectSchema = Joi.object({
...
export const createMilestoneSchema: Joi.ObjectSchema = Joi.object({
  name: Joi.string().trim().max(200).required(),
  description: Joi.string().trim().max(5000).allow(''),
  dueDate: Joi.date(),
});

export const updateMilestoneSchema: Joi.ObjectSchema = Joi.object({
  name: Joi.string().trim().max(200),
  description: Joi.string().trim().max(5000).allow(''),
  dueDate: Joi.date(),
...
export const createTimesheetSchema: Joi.ObjectSchema = Joi.object({
  projectId: objectId.required(),
  taskId: objectId,
  date: Joi.date().required(),
  hours: Joi.number().min(0.25).max(24).required(),
